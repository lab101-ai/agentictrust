...
class agentictrust_property:
    """
    A descriptor that provides a standardized way to handle agent property access and storage.
    Properties are automatically stored with an '_agentictrust_' prefix to avoid naming conflicts.
    ...
    Attributes:
        private_name (str): The internal name used for storing the property value,
            prefixed with '_agentictrust_'. Set either through __init__ or __set_name__.

    Example:
        ```python
        # Direct usage in a class
        class Agent:
            name = agentictrust_property()
            id = agentictrust_property()

            def __init__(self):
                self.name = "Agent1"  # Stored as '_agentictrust_name'
                self.id = "123"       # Stored as '_agentictrust_id'

        # Usage with decorator
        @track_agent()
        class Agent:
            pass
            # agentictrust_agent_id and agentictrust_agent_name are added automatically
        ```

    Notes:
        - Property names with 'agentictrust_' prefix are automatically stripped when creating
    ...
    def __init__(self, name=None):
        ...
        if name:
            self.private_name = f"_agentictrust_{name.replace('agentictrust_', '')}"
    ...
    def stack_lookup() -> Union[UUID, None]:
        ...
        This method searches the call stack for objects that have agentictrust_property
        ...
        for frame_info in inspect.stack():
            ...
            for var_name, var in local_vars.items():
                ...
                try:
                    # Check if object has our AgenticTrustDescriptor descriptors
                    var_type = type(var)

                    # Get all class attributes
                    class_attrs = {name: getattr(var_type, name, None) for name in dir(var_type)}

                    agent_id_desc = class_attrs.get("agentictrust_agent_id")

                    if isinstance(agent_id_desc, agentictrust_property):
                        agent_id = agent_id_desc.__get__(var, var_type)

                        if agent_id:
                            agent_name_desc = class_attrs.get("agentictrust_agent_name")
                            if isinstance(agent_name_desc, agentictrust_property):
