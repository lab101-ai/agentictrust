...
class HttpClient:
    ...
    def _prepare_headers(
        ...
        if api_key is not None:
            headers["X-agentictrust-Api-Key"] = api_key

        if parent_key is not None:
            headers["X-agentictrust-Parent-Key"] = parent_key
    ...
    def post(
        ...
        if result.code == 401:
            raise ApiServerException(
                f"API server: invalid API key: {api_key}. Find your API key at https://app.agentictrust.ai/settings/projects"
            )
        if result.code == 400:
